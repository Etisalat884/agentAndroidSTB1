...
def compare_video_image(img1, img2):
    hash0 = imagehash.average_hash(Image.open(img1))
    hash1 = imagehash.average_hash(Image.open(img2))

    cutoff = 5  # allow up to 5 bits difference
    diff = hash0 - hash1
    print("Image difference:", diff)

    if diff < cutoff:
        return True  # images are considered identical (paused)
    else:
        return False  # images are different (playing)
...
def restore_ffmpeg(device="/dev/video1"):
    """
...
def is_playback_active(capture_func, check_duration=5, threshold=10):
    """
    Validates playback by comparing screen frames over a few seconds.

    Args:
        capture_func: A function that captures and returns the current screen frame (as a numpy array).
        check_duration: Total duration (in seconds) to validate playback.
        threshold: Difference threshold to consider playback active.

    Returns:
        bool: True if playback appears active, False otherwise.
    """
    prev_frame = capture_func()
    time.sleep(1)
    start_time = time.time()

    while time.time() - start_time < check_duration:
        current_frame = capture_func()
        diff = cv2.absdiff(prev_frame, current_frame)
        non_zero_count = np.count_nonzero(diff)

        if non_zero_count > threshold * 1000:  # You can tune this value
    ...
    print("Playback is frozen or not active")
    return False

def capture_screen():
    # Capture the screen from STB or emulator (customize as needed)
    import pyautogui
    screenshot = pyautogui.screenshot()
    return cv2.cvtColor(np.array(screenshot), cv2.COLOR_RGB2BGR)


# capture_image_run(1, '/home/ltts/Documents/evqual_automation/agentAndroidSTB1/workspace/Images/ref3_replace.jpg')