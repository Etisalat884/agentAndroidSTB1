...
# -----------------------------------
def crop_region(image_path, y1_ratio, y2_ratio, x1_ratio=0.47, x2_ratio=0.75, tag="crop"):
...
def get_device_information():
    ...
    # Save Path
    base_dir = "/home/ltts2/Documents/evqual_automation/agentAndroidSTB1/workspace/Lib/Python/STB/STB04_KSTB6080"
    os.makedirs(base_dir, exist_ok=True)
    ...
    firmware_img = crop_region(screenshot_path, 0.36, 0.41, tag="firmware")
    serial_img   = crop_region(screenshot_path, 0.42, 0.46, tag="serial")
    model_img    = crop_region(screenshot_path, 0.65, 0.70, tag="model")
    ...
    firmware_text = extract_text(firmware_img)
    ...
    # Optional Cleaning
    firmware_clean = re.sub(r'[^0-9.]', '', firmware_text)
    serial_clean   = re.sub(r'[^0-9A-Za-z]', '', serial_text)
    model_clean    = model_text.strip()
    sap_clean      = sap_text.strip()
    app_clean      = app_text.strip()
    ...
    press("HOME",1)
    return {
        "Firmware Version": firmware_clean,
        "Serial Number": serial_clean,
        "STB Model": model_clean,
        "SAP Version": sap_clean,
        "Application Version": app_clean
    }
